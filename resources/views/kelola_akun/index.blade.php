<x-app-layout>
    ...
    <div class="py-12 bg-[#f8fafc] dark:bg-gray-900 min-h-screen">
                        ...
                        <table class="w-full text-left border-separate border-spacing-y-3">
                            <thead>
                                <tr class="text-[11px] font-black uppercase tracking-[0.2em] text-gray-400">
                                    ...
                                    <th class="px-6 py-4 text-center">NISN (Siswa)</th>
                                    <th class="px-6 py-4 text-right">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y-8 divide-transparent">
                                @forelse ($users as $user)
                                <tr class="group bg-white dark:bg-gray-800/50 hover:shadow-xl hover:shadow-gray-200/40 transition-all duration-300">
                                    <td class="px-6 py-6 bg-gray-50/50 dark:bg-gray-700/30 rounded-l-[1.5rem] border-y border-l border-gray-50 dark:border-gray-700">
                                        <div class="flex items-center">
                                            <div class="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center font-black text-xs mr-4 uppercase">
                                                {{ substr($user->name, 0, 2) }}
                                            </div>
                                            <div class="text-sm font-black text-gray-900 dark:text-white leading-none">{{ $user->name }}</div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-6 text-center border-y border-gray-50 dark:border-gray-700">
                                        @if($user->role == 'admin')
                                            <span class="px-4 py-1.5 bg-purple-100 text-purple-700 rounded-full text-[10px] font-black uppercase tracking-widest">Administrator</span>
                                        @elseif($user->role == 'petugas')
                                            <span class="px-4 py-1.5 bg-blue-100 text-blue-700 rounded-full text-[10px] font-black uppercase tracking-widest">Petugas</span>
                                        @else
                                            <span class="px-4 py-1.5 bg-amber-100 text-amber-700 rounded-full text-[10px] font-black uppercase tracking-widest">Siswa</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-6 text-center border-y border-gray-50 dark:border-gray-700">
        ...
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                ...
                                @empty
                                <tr>
                                    <td colspan="5" class="py-20 text-center text-gray-400 font-bold italic">Belum ada akun terdaftar.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
